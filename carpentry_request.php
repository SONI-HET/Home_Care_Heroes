...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST["laundry_service_provider"], $_POST["carpentry_service"], $_POST["pickup_date"], $_POST["pickup_time"])) {
        ...
        $sql = "SELECT s.service_name, p.price
                FROM carpentry_services s
                INNER JOIN service_prices p ON s.id = p.service_id
            ...
            if ($executionResult) {
                ...
                if (mysqli_stmt_num_rows($stmt) > 0) {
                    ...
                    if ($fetchResult) {
                        ...
                        $insertSql = "INSERT INTO carpentry_request (order_id, user_email, username, phone_number, address, service_provider_name, service, price, request_status, appointment_date, appointment_time) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
                        ...
                        if ($insertStmt) {
                            ...
                            if ($executionResult) {
                                ...
                                echo "<script>alert('Carpentry Request Sent Successfully!');</script>";
                            } else {
...
<head>
  <meta charset="utf-8">
  <title>Carpentry Request Form</title>
  <style>
    body {
    ...
    .button {
      ...
      font-size: 18px;
      border-radius: 8px;
      margin-right: 20px; /* Reduce margin-right */
...
<body>
  ...
  <h2>Carpentry Request Form</h2>
  <form class="" action="" method="post">
    <label for="laundry_service_provider">Choose Carpenter:</label>
    <select name="laundry_service_provider" id="laundry_service_provider" required>
      <option value="">Select</option>
      <?php
      $sql = "SELECT * FROM users WHERE user_type = 'carpenter'";
    ...
    <label for="carpentry_service">Choose Carpentry Service:</label>
    <select name="carpentry_service" id="carpentry_service" required>
      <option value="">Select Carpenter First</option>
    </select>
    ...
    <a href="./carpentry_service.php" style="text-decoration: none; margin-right: 20px;">Go back</a>
  </form>
  ...
  <script>
    $(document).ready(function() {
      $('#laundry_service_provider').change(function() {
        ...
        $.ajax({
          url: 'fetch_carpentry_services.php',
          type: 'get',
          data: {
            'carpenter': carpenter
          },
